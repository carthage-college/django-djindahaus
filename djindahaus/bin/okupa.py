...
from djindahaus.core.models import Capacity
...
def main():
    ...
    for idx, domain in enumerate(domains):
        print(domain)
        zona = Area.objects.get(rf_domain=domain['id'])
        capacity = Capacity.objects.create(area=zona, capacity=zona.capacity)
        # auth token from NAC
        headers = {
        ...
        if devices:
            ...
            # update our capacity data object
            ...
            if domains[idx]['areas']:
                ...
                for aid, _ in enumerate(domains[idx]['areas']):
                    zona = Area.objects.get(
                        rf_domain=domains[idx]['areas'][aid]['id'],
                    )
                    capacity = Capacity.objects.create(
                        area=zona, capacity=zona.capacity,
                    )
